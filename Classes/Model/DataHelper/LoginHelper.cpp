...
void LoginHelper::onReceiveServerResponse(Message* pMsg)
{
	...
	case MessageType::ApplyLogin:
	{
		...
		UIManager::getInstance()->SendEvent(DialogID::Login,UIEvent::EVENT_LOGIN_RESULT,pMsg->m_bIsSuccess);
	}
	break;
	case MessageType::ChipsChange:
	{
		...
		UIManager::getInstance()->SendEvent(DialogID::Max, UIEvent::EVENT_USER_CHIPS_CHANGE, pMsg->m_bIsSuccess,pResponse->m_nChips);
	}
	break;
	default:
		break;
...
void LoginHelper::sendApplyLogin(const std::string& szAccount, const std::string& szPwd)
{
	...
	sendHttpRequest<UserInfo>(pMsg, [](const UserInfo& info){
		...
		UIManager::getInstance()->SendEvent(DialogID::Login, UIEvent::EVENT_LOGIN_RESULT, true);
	});
