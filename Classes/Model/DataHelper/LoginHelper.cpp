...
void LoginHelper::onReceiveServerResponse(Message* pMsg)
{
	...
	case MessageType::ApplyLogin:
	{
		...
		{
			CurrentPlayerInfo::getInstance()->getUserInfo().setAccount(pResponse->m_szAccount);
			CurrentPlayerInfo::getInstance()->getUserInfo().setChips(pResponse->m_nChips);
			CurrentPlayerInfo::getInstance()->getUserInfo().setName(pResponse->m_szName);
		}
...
void LoginHelper::sendApplyLogin(const std::string& szAccount, const std::string& szPwd)
{
	...
	sendHttpRequest<UserInfo>(pMsg, [](const UserInfo& info){
		CurrentPlayerInfo::getInstance()->setUserInfo(info);
